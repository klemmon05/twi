...
import { isDatabaseUrlLikelyValid } from "@/lib/db";

export const dynamic = "force-dynamic";

export default async function HomePage() {
  if (!isDatabaseUrlLikelyValid()) {
    return (
      <section className="card space-y-2">
        <h1 className="text-2xl font-semibold">ProSignal Analytics</h1>
        <p className="text-slate-300">The application is online, but the database URL is not configured correctly yet.</p>
        <p className="text-sm text-slate-400">Set a full DATABASE_URL in Vercel (with the real password), redeploy, then run npx prisma migrate deploy.</p>
      </section>
    );
  }
  try {
